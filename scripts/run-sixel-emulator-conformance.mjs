...
try {
  ...
  if (tmux !== undefined) {
    ...
    await waitUntil(async () => (await colorPixelsAfterScreenshot(windowId, 'graphics-refreshed.png')).graphic.count > 100,
      'tmux SIXEL redraw');
  }
  ...
  assert.ok(hiddenPixels.graphic.count < 10, 'xterm retained saturated SIXEL pixels after image removal.');
...
function assertGraphicAboveScrollBoundary(pixels, cellPixels, rows) {
  ...
  assert.ok(
    Math.abs(pixels.graphic.maxY - expectedBottom) <= 3,
    `SIXEL graphic ended at pixel ${String(pixels.graphic.maxY)} instead of the safe scroll boundary ${String(expectedBottom)}.`,
  );
}
