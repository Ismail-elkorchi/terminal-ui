...
export function calendarReducer(
  ...
  switch (transition.kind) {
    case 'select':
      return selectable(transition.date, options)
        ? { ...state, selectedDate: transition.date, activeDate: transition.date, visibleMonth: monthOf(transition.date) }
        : state;
    case 'setActive':
      return selectable(transition.date, options)
        ? { ...state, activeDate: transition.date, visibleMonth: monthOf(transition.date) }
        : state;
    case 'moveActive':
      return moveActive(state, transition.days, options);
    case 'commitActive':
      return state.activeDate === undefined || !selectable(state.activeDate, options)
        ? state
        : { ...state, selectedDate: state.activeDate };
    case 'moveMonth':
      return moveVisibleMonth(state, transition.months, options);
    case 'startOfWeek':
...
export function calendarView(
  ...
  const visibleActiveId = activeId === undefined
    ? undefined
  ...
  return {
    ...
    days,
    interaction: {
      ...(visibleActiveId === undefined ? {} : { activeId: visibleActiveId }),
...
function moveVisibleMonth(
  ...
  const active = nearestVisibleSelectableDate(visibleMonth, candidate, options);
...
function nearestVisibleSelectableDate(
  visibleMonth: CalendarMonth,
