...
export function notificationReducer(
  ...
  const normalized = normalizeCapacity(state, policy, transitionTime(transition));
  switch (transition.kind) {
    case 'enqueue':
      return enqueue(normalized, transition.notification, transition.now, transition.conflict ?? 'keep-existing', policy);
    case 'replace':
      return replace(normalized, transition.id, transition.notification, transition.now, policy);
    case 'update':
      return update(normalized, transition.id, transition.changes, transition.now);
    case 'dismiss':
      return dismiss(normalized, transition.id, transition.now, policy);
    case 'pause':
      return mapNotification(normalized, transition.id, (record) => pause(record, transition.now));
    case 'resume':
      return mapNotification(normalized, transition.id, (record) => resume(record, transition.now));
    case 'expire':
      return expire(normalized, transition.now, policy);
    case 'selectHistory':
      return normalized.history.some((entry) => entry.notification.id === transition.id)
        ? withHistorySelection(normalized, transition.id)
        : normalized;
    case 'moveHistorySelection':
      return moveHistorySelection(normalized, transition.delta);
    case 'firstHistory':
    ...
    case 'setHistoryView':
      if (!normalized.history.some((entry) => entry.notification.id === transition.selectedId)) return normalized;
      return withHistoryScroll(withHistorySelection(normalized, transition.selectedId), ownScroll(transition.scroll));
    case 'scrollHistory':
      return withHistoryScroll(normalized, ownScroll(transition.scroll));
    case 'removeHistory':
      return removeHistory(normalized, transition.id);
    case 'clear':
      return clear(normalized, transition.now, policy);
    case 'clearHistory':
...
export function notificationTransitionFromHistory(
  transition: NotificationHistoryTransition,
  now: number
  ...
  switch (transition.kind) {
    case 'selection':
      return {
        ...
        scroll: ownScroll(transition.scroll),
        now: finiteTime(now),
      };
    case 'scroll':
      return { kind: 'scrollHistory', scroll: ownScroll(transition.scroll), now: finiteTime(now) };
    case 'remove':
      return { kind: 'removeHistory', id: transition.id, now: finiteTime(now) };
  }
}
...
function transitionTime(transition: NotificationTransition): number {
  return finiteTime(transition.now);
}
...
function finiteTime(value: number): number {
  if (!Number.isFinite(value)) throw new TypeError('Notification transition time must be finite.');
  return value;
}
