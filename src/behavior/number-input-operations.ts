...
export function numberInputReducer(
  ...
  transition: NumberInputTransition
): NumberInputState {
  switch (transition.kind) {
    case 'edit': {
      const input = editTextBuffer(state.input, singleLineOperation(transition.operation));
      return input === state.input ? state : { ...state, input };
    }
    case 'pointer': {
      const input = applyTextPointerTransition(state.input, transition.transition);
      return input === state.input ? state : { ...state, input };
    }
    case 'step':
      return steppedState(state, transition.direction);
    case 'commit':
...
export function numberInputView(state: NumberInputState): NumberInputView {
