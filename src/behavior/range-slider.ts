...
export function rangeSliderReducer(
  ...
  switch (action.kind) {
    case 'selectHandle':
      return normalized.activeHandle === action.handle ? normalized : { ...normalized, activeHandle: action.handle };
    case 'step': {
      const direction = action.direction === 'increment' ? 1 : -1;
