import type { SplitPaneTransition } from './split-pane.ts';
...
export function splitPaneReducer(
  ...
  transition: SplitPaneTransition,
  ...
  switch (transition.kind) {
    case 'setActiveDivider':
      return validDivider(transition.dividerIndex, dividerCount)
        ? withActiveDivider(state, shares, transition.dividerIndex)
        : state;
    case 'moveActiveDivider':
      return withActiveDivider(
        state,
        shares,
        clampDivider((state.activeDivider ?? 0) + Math.sign(transition.delta), dividerCount)
      );
    ...
    case 'resizeBy':
      return resizeState(state, shares, state.activeDivider ?? 0, transition.deltaShare, options.constraints);
    case 'beginResize':
      return validDivider(transition.dividerIndex, dividerCount)
        ? {
            ...withActiveDivider(state, shares, transition.dividerIndex),
            drag: { dividerIndex: transition.dividerIndex, anchorShares: shares }
          }
        : state;
    case 'resizeFromAnchor': {
      const drag = state.drag;
      if (drag?.dividerIndex !== transition.dividerIndex) return state;
      return resizeState(state, drag.anchorShares, transition.dividerIndex, transition.deltaShare, options.constraints);
    }
    case 'endResize':
      return state.drag?.dividerIndex === transition.dividerIndex ? withoutDrag(state) : state;
  }
}

export function splitPaneLayout(state: SplitPaneState): SplitPaneLayout {
  const shares = normalizeShares(state.shares.length, state.shares);
  return {
