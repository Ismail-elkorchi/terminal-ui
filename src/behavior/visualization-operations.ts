...
export function chartReducer(
  ...
  if (currentSeries === undefined) return normalizeCollectionInteraction(state, createCollectionInteractionIndex(ids));
...
export function heatmapReducer<TValue>(
  ...
  if (current === undefined) return normalizeCollectionInteraction(state, createCollectionInteractionIndex(ids));
...
function reduceCollection(
  ...
  return collectionInteractionReducer(state, transition, {
    index: createCollectionInteractionIndex(enabledIds),
...
): transition is CollectionInteractionTransition {
