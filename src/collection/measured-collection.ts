...
/** @beta */
export function createMeasuredCollection<TValue>(
  items: readonly MeasuredCollectionItem<TValue>[]
): MeasuredCollection<TValue> {
  const batch = decodeMeasuredItemArray(items, undefined);
  return measuredCollectionFromIndexes(
    buildSequence(batch.items, 0, batch.items.length, 0n),
    batch.ids
  );
}
...
export function appendMeasuredItems<TValue>(
  ...
  const batch = decodeMeasuredItemArray<TValue>(items, data.ids, startOrder);
  if (batch.items.length === 0) return collection;
  assertTotalRows(collection.totalRows, batch.totalRows);
  const appended = buildSequence(batch.items, 0, batch.items.length, startOrder);
  return measuredCollectionFromIndexes(joinSequences(data.sequence, appended), batch.ids);
}
...
export function prependMeasuredItems<TValue>(
  ...
  const batch = decodeMeasuredItems(items, data.ids, startOrder);
  if (batch.items.length === 0) return collection;
  assertTotalRows(collection.totalRows, batch.totalRows);
  const prepended = buildSequence(batch.items, 0, batch.items.length, startOrder);
  return measuredCollectionFromIndexes(joinSequences(prepended, data.sequence), batch.ids);
}
...
export function removeMeasuredItems<TValue>(
  ...
  return changed ? measuredCollectionFromIndexes(sequence, idIndex) : collection;
}
...
function decodeMeasuredItemArray<TValue>(
  supplied: readonly MeasuredCollectionItem<TValue>[],
  existingIds: IdIndexNode | undefined,
  startOrder = 0n
): ReturnType<typeof decodeMeasuredItems<TValue>> {
  assertMeasuredItems(supplied);
  return decodeMeasuredItems(supplied, existingIds, startOrder);
}
...
function decodeMeasuredItems<TValue>(
  supplied: readonly MeasuredCollectionItem<TValue>[],
  existingIds: IdIndexNode | undefined,
  startOrder = 0n
): {
  ...
  for (const [index, item] of supplied.entries()) {
    const decodedItem = decodeMeasuredItem<TValue>(item, `Measured collection items[${String(index)}]`);
    if (idIndexGet(ids, decodedItem.id) !== undefined) {
      throw new TypeError('Measured collection item ids must be unique.');
    }
    totalRows = checkedRowTotal(totalRows, decodedItem.rows);
    const order = startOrder + BigInt(index);
    ids = idIndexSet(ids, Object.freeze({ id: decodedItem.id, hash: hashId(decodedItem.id), order }), 0);
...
function decodeMeasuredItem<TValue>(value: MeasuredCollectionItem<TValue>, subject: string): MeasuredCollectionItem<TValue> {
...
function measuredCollectionFromIndexes<TValue>(
...
function measuredCollectionData<TValue>(
  ...
  if (data === undefined) {
    throw new TypeError('Measured collection must be created with createMeasuredCollection().');
  }
