...
function processStatusStyle(status: ProcessStatus): import('../../visual/render-content.ts').TerminalStyle {
  const token = status === 'running'
    ? 'status.running'
    : status === 'success'
    ? 'status.success'
...
function assertChartDataStatus(
  value: unknown,
  component: string,
