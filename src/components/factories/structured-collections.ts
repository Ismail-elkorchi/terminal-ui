...
import type { CollectionSnapshot, CollectionItem } from '../../collection/snapshot.ts';
import {
...
import type { TableColumn, TableColumnWidth } from '../table-column.ts';
import type { TableStylePart, TreeStylePart } from '../style-parts.ts';
...
function createTableModel<TRow, TMessage extends ComponentMessage>(
...
function decodeTableState(
  ...
  const activeRowId = interaction?.kind === 'row'
    ? decodeOptionalId(interaction.activeRowId, 'dataGrid activeRowId')
  ...
  const selectedRowIds = interaction?.kind === 'row'
    ? interaction.selection.mode === 'multiple'
      ? decodeUniqueIds(interaction.selection.selectedRowIds, 'dataGrid selectedRowIds')
  ...
  const selectedCells = interaction?.kind === 'cell'
    ? interaction.selection.mode === 'multiple'
      ? decodeGridCells(interaction.selection.selectedCells, 'dataGrid selectedCells')
      : interaction.selection.mode === 'single' && interaction.selection.selectedCell !== undefined
        ? decodeGridCells([interaction.selection.selectedCell], 'dataGrid selectedCell')
        : Object.freeze([])
    : Object.freeze([]);
  let sort:
...
function tableSourceFor(model: TableModel): TableRenderSource {
  const source = tableSources.get(model.source);
  if (source === undefined) throw new TypeError('table render source is unavailable.');
  return source;
}

function tableRowModel(model: TableModel, localIndex: number): TableRowModel {
  const source = tableSourceFor(model);
