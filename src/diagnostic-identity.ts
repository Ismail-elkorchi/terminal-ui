...
export function sha256Hex(value: string): string {
  const message = paddedMessage(new TextEncoder().encode(value));
...
function paddedMessage(input: Uint8Array): Uint8Array {
  ...
  view.setUint32(byteLength - 8, Number((bitLength >> 32n) & 0xffffffffn));
  view.setUint32(byteLength - 4, Number(bitLength & 0xffffffffn));
  return message;
}
