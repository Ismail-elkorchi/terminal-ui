...
function scrollMessageFactory<TMessage>(
  renderNode: ScrollableNode<TMessage>
): ((request: ScrollRequest) => MessageResolution<TMessage>) | undefined {
...
function scrollContentHitTarget<TMessage>(
  ...
  factory: (request: ScrollRequest) => MessageResolution<TMessage>
  ...
  return {
    ...
    accepts: ['scroll'],
    message: (event) => {
      const transition = scrollTransitionForWheel(event, wheel);
      return transition === undefined ? ignoreMessage() : factory(scrollRequest(transition, state, event, 'content'));
    },
...
function scrollbarTrackHitTargets<TMessage>(
  ...
  const trackTarget: HitTarget<TMessage> & ScrollRoutable<TMessage> = {
    ...
    cursor: 'pointer',
    message: (event) => {
      const transition = event.kind === 'scroll'
        ? scrollTransitionForWheel(event, wheel)
        : scrollTransitionForTrack(axis, track, state, event);
      return transition === undefined ? ignoreMessage() : factory(scrollRequest(
        transition,
        state,
        event,
  ...
  return thumbBounds === undefined
    ...
    : [
        trackTarget,
        {
          ...
          cursor: 'pointer',
          message: (event) => {
            const transition = scrollTransitionForThumb(axis, track, state, event);
            return transition === undefined ? ignoreMessage() : factory(scrollRequest(
              transition,
              state,
              event,
...
function wheelRoute<TMessage>(
  ...
  return Object.freeze({
    ...
    route(pointer: RoutedPointerEvent, state: ScrollState) {
      const transition = scrollTransitionForWheel(pointer, policy);
      const nextState = transition === undefined
        ? state
        : scrollReducer(state, transition, scrollGeometry(initial));
      return Object.freeze({
        nextState,
        message: transition === undefined
          ? ignoreMessage()
          : factory({ nextState, source: scrollRequestSource(pointer), target }),
      });
    },
  });
}
...
function scrollTransitionForWheel(
...
function scrollTransitionForTrack(
  axis: 'vertical' | 'horizontal',
  track: ScrollbarTrack,
  state: ScrollbarState,
  event: RoutedPointerEvent
...
function scrollTransitionForThumb(
  axis: 'vertical' | 'horizontal',
  track: ScrollbarTrack,
  state: ScrollbarState,
  event: RoutedPointerEvent
...
export function viewportKeyboardScrollMessage<TMessage>(
  ...
  const transition = keyboardScrollTransition(event.key, policy);
  const factory = scrollMessageFactory(renderNode);
  if (transition === undefined || factory === undefined) return ignoreMessage();
  ...
  const nextState = scrollReducer(scrollPosition(state), transition, scrollGeometry(state));
...
function keyboardScrollTransition(
