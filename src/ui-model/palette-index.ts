...
const paletteIndexBrand: unique symbol = Symbol('terminal-ui.palette-index');
const queryCacheLimit = 32;

export interface PaletteIndex<TValue = string> {
  readonly [paletteIndexBrand]: TValue;
  readonly kind: 'palette-index';
...
interface PaletteIndexData<TValue> {
  readonly entries: readonly SearchEntry<TValue>[];
  readonly searchable: readonly (readonly string[])[];
  readonly projections: Map<string, PaletteQueryProjection<TValue>>;
  queryEvaluations: number;
  candidateEvaluations: number;
}

const sourceIndexes = new WeakMap<object, PaletteIndex<unknown>>();
const indexData = new WeakMap<object, PaletteIndexData<unknown>>();

export function preparePaletteIndex<TValue>(entries: readonly SearchEntry<TValue>[]): PaletteIndex<TValue> {
  const cached = sourceIndexes.get(entries) as PaletteIndex<TValue> | undefined;
  if (cached !== undefined) return cached;
  const seen = new Set<string>();
  const normalized = Object.freeze(entries.map((entry): SearchEntry<TValue> => {
    const id = clean(entry.id);
    if (id.length === 0) throw new TypeError('palette entry ids must not be empty.');
    if (seen.has(id)) throw new TypeError(`palette entry ids must be unique; duplicate id: ${id}`);
    seen.add(id);
  ...
  const index = Object.freeze<PaletteIndex<TValue>>({
    [paletteIndexBrand]: undefined as TValue,
    kind: 'palette-index',
...
export function projectPaletteQuery<TValue>(
  index: PaletteIndex<TValue>,
  query = ''
): PaletteQueryProjection<TValue> {
...
export function paletteIndexStatistics(index: PaletteIndex<unknown>): {
...
function dataFor<TValue>(index: PaletteIndex<TValue>): PaletteIndexData<TValue> {
  const data = indexData.get(index) as PaletteIndexData<TValue> | undefined;
  if (data === undefined) throw new TypeError('palette indexes must be created with preparePaletteIndex().');
  return data;
}
