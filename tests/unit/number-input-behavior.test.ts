...
void test('number input reducer keeps editable invalid text and commits reverts and steps explicitly', () => {
...
void test('number input configuration owns grammar-aware formatting and validation', () => {
...
void test('number input renders controlled text validity and pointer step controls', () => {
  ...
  assert.ok(increment);
  assert.deepEqual(increment.message(routedPointerEvent()), { kind: 'step', direction: 'increment' });
