...
import type { TreeNode } from '../../dist/components/index.js';

void test('treeReducer models lazy pending success error and empty states', () => {
  const nodes = [{
    id: 'root',
    label: 'Root',
  ...
  }] satisfies readonly TreeNode<{ readonly path: string }>[];
  ...
  const pendingRoot = pending.nodes[0];
  const failedRoot = failed.nodes[0];
  const loadedRoot = loaded.nodes[0];
  const emptyRoot = empty.nodes[0];
  assert.ok(pendingRoot?.kind === 'lazy');
  assert.ok(failedRoot?.kind === 'lazy');
  assert.ok(loadedRoot?.kind === 'branch');
  assert.ok(emptyRoot?.kind === 'lazy');
  assert.deepEqual(pendingRoot.loading.kind, 'pending');
  assert.deepEqual(failedRoot.loading.kind, 'error');
  assert.equal(loadedRoot.children[0]?.label, 'Child');
  assert.equal(loadedRoot.description, 'workspace/root');
...
void test('treeReducer renames nodes without mutating input', () => {
  const nodes = [{ id: 'root', label: 'Root', kind: 'leaf' }] satisfies readonly TreeNode[];
...
void test('treeReducer owns hierarchy selection filter and rename state together', () => {
  const initial = {
    nodes: [{ id: 'node-a', label: 'Node A', kind: 'leaf' }]
  } satisfies { readonly nodes: readonly TreeNode[] };
...
void test('treeNodeMatches searches label id description icon lazy message and metadata', () => {
  const node = {
    id: 'api',
    label: 'Service',
  ...
  } satisfies TreeNode<{ readonly domain: string }>;
...
void test('visibleTreeRows exposes depth paths expanded children and lazy placeholders', () => {
  const nodes = [{
    id: 'root',
    label: 'Root',
    kind: 'branch',
...
void test('visibleTreeRows keeps matching descendants and their ancestors while filtering', () => {
  const nodes = [{
    id: 'root',
    label: 'Root',
    kind: 'branch',
...
void test('nextTreeRowId skips disabled and lazy placeholder rows and wraps', () => {
  const nodes = [{
    id: 'root',
    label: 'Root',
    kind: 'branch',
...
void test('treeDisclosureAction returns only meaningful disclosure actions', () => {
  const leaf = { id: 'leaf', label: 'Leaf', kind: 'leaf' } satisfies TreeNode;
  const collapsed = {
    id: 'folder',
    label: 'Folder',
    kind: 'branch',
  ...
  const expanded: TreeNode = { ...collapsed, expanded: true };
  const lazy = {
    id: 'lazy',
    label: 'Lazy',
    kind: 'lazy',
